-- Migration 003 — rebase ratings on 500, with larger point swings.
--
-- Run this once in the Supabase SQL editor IF you already ran schema.sql or
-- migration 002. A fresh project only needs schema.sql.
--
-- Every existing rating simply moves up by 400. The scale is unchanged, so all
-- the gaps between players are preserved exactly: a player 30 points ahead
-- stays 30 points ahead. Only the numbers get bigger, which is the point —
-- games are now worth about four times as much, so rounding costs far less.
...
-- 2. Move everyone onto the new base and change the starting point.
update public.profiles set rating = rating + 400;

alter table public.profiles
  alter column rating set default 500;

-- 3. Put the policy back, now expecting the new starting rating.
...
-- 5. Replace the scoring function, which now uses k = 40.
